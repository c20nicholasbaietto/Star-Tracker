...
def solve_image(directory, filepath, pic_num, MEDIAN_IMAGE, my_star_db, star_position, num_stars):
    ...
    for c in contours:
        ...
        if M['m00'] > 0:
            ...
            img_stars += beast.star(cx - width / 2.0, (cy - height / 2.0), float(cv2.getRectSubPix(img_grey, (1,1), (cx,cy))[0,0]), -1)
	...
	#q = str(q4)+" "+str(q1)+"i "+str(q2)+"j "+str(q3) <- old one didn't have k term.
        ...
        erythin = q + ";" + str(DEC*(180/math.pi)) + ";" + str(RA*(180/math.pi)) + ";" + str(ORI*(180/math.pi)) + ";" + \
