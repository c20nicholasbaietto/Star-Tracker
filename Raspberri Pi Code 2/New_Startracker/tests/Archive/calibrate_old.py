...
if __name__ == '__main__':
    ...
    images = np.asarray([cv2.imread( join(samplepath,image_names[n]) ).astype(np.float32) for n in range(0, num_images)])
    ...
    for n in range(0, num_images):
        ...
        if isfile(basename(image_name)+'.wcs'):
            # if a picture was solved (can use to count how many pictures were successful)
