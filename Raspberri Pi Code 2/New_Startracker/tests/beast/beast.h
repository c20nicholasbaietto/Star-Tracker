...
public:
	...
	*/
	float * print_ori() {
		...
		fprintf(stderr,"ORIENTATION=%f\n",180 - (-atan2(R32,R33)* 180 / PI));
		float DEC = fmod(360+asin(R31)* 180 / PI,360);
		float RA = fmod(360+atan2(R21,R11)* 180 / PI,360);
		float ORIENTATION = 180 - (-atan2(R32,R33)* 180 / PI);
		static float attitude[3];
		attitude[0] = DEC;
		attitude[1] = RA;
		attitude[2] = ORIENTATION;
		return attitude;
