...
COM_Port = serial.Serial("/dev/ttyS0", baudrate=9600, bytesize=8, parity='N', stopbits=1,
                         timeout=300)  # open serial port
...
while True:
    ...
    image_name = file_path + "/test" + str(
        pic_num) + file_type  # define image name in while loop instead of outside so we can save all pictures

    image_text = file_path + "/test" + str(
        pic_num - 1) + ".txt"  # text file name with useful data (still working on not getting the last image)
    ...
    if command == "quit":
        ...
        if type(my_reply) is tuple:  # if the image was solved, this will be true
            ...
            solved = True
            print("Quaternian: " + reply[
                0])  # print the quaternion (this will also need to be returned to the calling function)
